...
export class UsersController {
  ...
  async getById(
    ...
    try {
      ...
      const requestingEmail = this.originEmail(req);
      const requestingUser = await this.usersService.findByEmail(requestingEmail);

      if (!requestingUser.isAdmin && requestingUser.id !== Number(id)) {
