...
function endpointSearchScope(
  repoPath: string,
  techStack: TechStack,
): { root: string; prefix: string } {
  const rawServiceRoot = (techStack.serviceRoot || ".")
    .replace(/\\/g, "/")
    .replace(/^\.\/?/, "")
    .replace(/\/+$/, "");
  ...
  try {
    ...
    // Fall back to repository-wide discovery below.
  }
...
function toRepoRelative(prefix: string, filePath: string): string {
...
function extractEndpointsFromFile(
  ...
  if (ext === ".ts" || ext === ".js") {
    ...
    // NestJS gRPC-Web: @GrpcMethod('ServiceName', 'MethodName')
    // Exposed as POST /grpc/{ServiceName}/{MethodName} (package prefix resolved from .proto files)
  ...
  // ---- Protocol Buffers (gRPC-Web over HTTP) ----
  if (ext === ".proto") {
    // Parse package, service, and rpc definitions to produce POST endpoints
    // gRPC-Web URL pattern: POST /grpc/{package}.{Service}/{Method}
