...
async function detectTechStackFromFiles(repoPath: string): Promise<TechStack> {
  ...
  const readJson = (rel: string) => {
    try { return JSON.parse(readFileSync(resolve(repoPath, rel), "utf-8")); }
    catch { return null; }
  };
  ...
  if (has("requirements.txt") || has("pyproject.toml") || has("setup.py") || has("Pipfile")) {
    ...
    const readReqs = () => {
      for (const f of ["requirements.txt", "Pipfile"]) {
        try { return readFileSync(resolve(repoPath, f), "utf-8").toLowerCase(); } catch { /* skip */ }
      }
      try { return readFileSync(resolve(repoPath, "pyproject.toml"), "utf-8").toLowerCase(); } catch { return ""; }
    };
    const reqs = readReqs();
...
const CONTROLLER_GLOBS = [
  // JS / TS
  "src/**/*.controller.{ts,js}", "src/**/routes.{ts,js}", "src/**/router.{ts,js}",
  "src/**/*.routes.{ts,js}", "app/controllers/**/*.{ts,js,rb}",
  "controllers/**/*.{ts,js}", "routes/**/*.{ts,js}", "api/**/*.{ts,js}",
  // .NET / C#
  "**/*Controller.cs", "**/*ApiController.cs", "**/Controllers/**/*.cs",
  // Java / Kotlin
  "**/*Controller.java", "**/*Controller.kt",
  "**/controller/**/*.java", "**/controllers/**/*.java",
  // Python
  "**/views.py", "**/routes.py", "**/api.py", "**/endpoints.py",
  "**/*_views.py", "**/*_routes.py", "**/urls.py",
  // Ruby
  "app/controllers/**/*.rb", "config/routes.rb",
  // Go
  "**/*handler*.go", "**/*router*.go",
  // PHP
  "**/Controller/**/*.php", "**/Controllers/**/*.php", "routes/**/*.php",
];
...
): { queryParams?: Array<{ name: string; value: string }>; body?: string; contentType?: string } | null {
    ...
    if (name === "grep_code") {
      ...
      try {
        const grepArgs = [
          ...
          "--exclude-dir=obj", "--exclude-dir=vendor",
          "-F", query, ".",
        ];
...
If you see a DTO/model type referenced, use find_type to look it up. Return JSON with body, contentType, queryParams, and pathParams.`,
    ...
    try {
      ...
      enriched.push({
        ...ep,
        path: resolvedPath,
        queryParams: parsed.queryParams?.length > 0
          ? parsed.queryParams
          : ep.queryParams,
