...
is_readiness_probe_step_name() {
  ...
  case "$normalized_name" in
    "probe application readiness"|"wait for application to be ready")
      return 0
      ;;
    *)
      return 1
...
find_readiness_probe_step_index() {
  local steps_dir dir base name_file step_name found=""
  ...
  for dir in "$steps_dir"/*; do
    ...
    if is_readiness_probe_step_name "$step_name"; then
      found="$base"
      break
    fi
  done
...
post_sync() {
  ...
  local url="${CI_AGENT_BASE_URL%/}/sync_step_session"
  local http_code
  local curl_exit
  local attempt=1
  ...
  while true; do
    ...
    if http_code="$(curl "${curl_args[@]}")"; then
      if [[ "$http_code" == "401" ]] && (( refreshed_oidc_token == 0 )); then
        log "sync_step_session returned HTTP 401; refreshing GitHub OIDC token and retrying once"
      ...
      case "$http_code" in
        408|429|500|502|503|504)
          if (( attempt < max_attempts )); then
            log "sync_step_session returned transient HTTP ${http_code}; retrying in ${backoff_seconds}s (attempt ${attempt}/${max_attempts})"
      ...
      if [[ ! "$http_code" =~ ^2 ]]; then
        log "sync_step_session returned HTTP ${http_code}"
    ...
    curl_exit=$?
    if (( attempt < max_attempts )); then
      log "sync_step_session request failed (curl exit ${curl_exit}); retrying in ${backoff_seconds}s (attempt ${attempt}/${max_attempts})"
      sleep "$backoff_seconds"
      attempt=$((attempt + 1))
      backoff_seconds=$((backoff_seconds * 2))
      continue
