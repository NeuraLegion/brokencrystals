...
async function brightGet(api, path2, query) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Bright API ${path2} returned HTTP ${res.status}: ${body.slice(0, 500)}`);
  }
...
async function getAuthObject(api, authObjectId) {
  return brightGet(api, `/api/v3/auth-objects/${encodeURIComponent(authObjectId)}`);
}
...
var dockerfileTools = [...codebaseTools, verifyDockerImageTool];
...
function looksLikeInternalCodeSearch(query) {
  return /(?:^|\s|["'`])(?:\.\/)?(?:apps|packages|services|libs|modules)\/[A-Za-z0-9._/-]+/i.test(
    query
...
function addAuthHint(hints, hint, opts = {}) {
  ...
  if (!compacted) return;
  if (hints.some(
    (existing) => existing === compacted || existing.includes(compacted) || compacted.includes(existing)
  )) {
    return;
  }
...
Evidence: ${verifiedTestUrl2.evidence}` : "\n\n### Verified auth test URL\nNo verified test URL was found. Use probe_url to find a protected endpoint that returns 401 without auth.";
    ...
    if (authObjectId2) {
      ...
      return {
        authObjectId: authObjectId2,
        hasAuth: true,
        authFailed: false,
        authHints,
        directAuthHeaders: capturedDirectHeaders2
      };
    }
...
Seeded credentials: username=${seededCredentials.username}, email=${seededCredentials.email}, password=${seededCredentials.password}. Use the app's actual login identifier field; do not mutate the stored username/email.`;
      ...
      if (!detection.loginEndpoint || isSetupLikeEndpoint(detection.loginEndpoint)) {
        const discovered = await discoverLoginEndpoint(
          baseUrl,
          detection.loginEndpoint ?? void 0
        );
        if (discovered) {
...
async function createAuthViaRestApi(api, projectId, repeaterId, params) {
  ...
  if (steps) {
    console.log(
      `[Auth] Auth object steps: ${steps.map((s) => `${s.name}(${s.request?.method} ${s.request?.url})`).join(" \u2192 ")}`
    );
  }
...
Supports multiple headers (e.g. both x-cal-client-id AND x-cal-secret-key).`,
  ...
  const customHandler = async (name, args) => {
    ...
    if (name === "create_auth_raw") {
      ...
      let successDetection = [{ type: "status", statuses: [200] }];
      ...
      const testUrl2 = normalizeAuthTestUrl(
        String(args.testUrl),
        baseUrl,
        detection,
        verifiedTestUrl
      ...
      console.log(
        `[Auth] Creating raw multistep auth \u2014 steps: ${stepNames}, test: ${testMethod} ${testUrl2}`
      );
      const result = await postAuthObject(api, body);
      if (result.error) {
        attemptLog.push(
          `- create_auth_raw(steps=[${stepNames}], testUrl=${testUrl2}) \u2192 ERROR: ${result.error}`
        );
        addAuthHint(
          authHints,
          `[auth-create-error] create_auth_raw failed for steps=[${stepNames}], testUrl=${testUrl2}: ${result.error}`
        );
        return JSON.stringify({ error: result.error });
      }
      return JSON.stringify({ authObjectId: result.id });
    ...
    if (name === "create_auth_oidc") {
      ...
      console.log(
        `[Auth] Creating ${grantLabel} auth \u2014 tokenEndpoint: ${tokenEndpoint}, clientId: ${clientId}, test: ${testUrl2}`
      );
      const result = await postAuthObject(api, body);
      if (result.error) {
        attemptLog.push(
          `- create_auth_oidc(grantType=${grantType}, tokenEndpoint=${tokenEndpoint}, clientId=${clientId}, testUrl=${testUrl2}) \u2192 ERROR: ${result.error}`
        );
    ...
    if (name === "create_auth_header") {
      ...
      console.log(
        `[Auth] Creating static header auth \u2014 headers: [${headerNames}], test: ${testMethod} ${testUrl2}`
      );
      const result = await postAuthObject(api, body);
      if (result.error) {
        attemptLog.push(
          `- create_auth_header(headers=[${headerNames}], testUrl=${testUrl2}) \u2192 ERROR: ${result.error}`
        );
...
\`\`\``
            ...
            const csrfInputMatch = rootBody.match(
...
**\u26A0\uFE0F CSRF TOKEN FOUND**: Hidden input field name="${csrfInputMatch[1]}" with a live token value. The login POST **requires** this field in the body. Use \`create_auth_raw\` with NexTemplate extraction.`
      ...
      if (isHtml) {
        const csrfInputMatch = getBody.match(
          /<input[^>]+type=["']hidden["'][^>]+name=["'](csrf|csrfmiddlewaretoken|_token|authenticity_token|_csrf_token|csrfToken)[^"']*["'][^>]*value=["']([^"']+)["']/i
        ) || getBody.match(
          /<input[^>]+name=["'](csrf|csrfmiddlewaretoken|_token|authenticity_token|_csrf_token|csrfToken)[^"']*["'][^>]+type=["']hidden["'][^>]*value=["']([^"']+)["']/i
        ) || getBody.match(
...
async function verifySeededCredentials(baseUrl, creds, detection) {
  ...
  if (!csrfToken) {
    ...
    for (const formUrl of formCsrfCandidates) {
      try {
        ...
        if (res.status === 200) {
          const body = await res.text();
          const csrfInputMatch = body.match(
            /<input[^>]+name=["'](csrf|csrfmiddlewaretoken|_token|authenticity_token|_csrf_token|csrfToken)[^"']*["'][^>]*value=["']([^"']+)["']/i
          ) || body.match(
            /<input[^>]+value=["']([^"']+)["'][^>]+name=["'](csrf|csrfmiddlewaretoken|_token|authenticity_token|_csrf_token|csrfToken)[^"']*["']/i
          );
          if (csrfInputMatch) {
            if (csrfInputMatch[2] && /^(csrf|csrfmiddlewaretoken|_token|authenticity_token|_csrf_token|csrfToken)$/i.test(
              csrfInputMatch[1]
            )) {
  ...
  try {
    ...
    const isNotActivated = /not.activated|not.verified|email.confirm|must.confirm|activation.required|verify.your.email/i.test(
      body
    );
    if (isNotActivated) {
      const preview = body.length > 200 ? body.slice(0, 200) + "..." : body;
        ...
        if (detection.protectedEndpointPath || detection.csrfRequired) {
          ...
          try {
            ...
            if (stillShowsLogin && verifyRes.status === 200) {
              return {
                valid: false,
                reason: "Login returned 302 but session was NOT authenticated \u2014 protected resource still shows login form (likely missing CSRF token in login POST)"
              };
            }
          } catch {
          }
...
//# sourceMappingURL=chunk-MJ45ILEP.js.map