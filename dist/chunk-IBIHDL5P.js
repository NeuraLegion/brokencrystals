...
var TokenTracker = class _TokenTracker {
  static instance;
  totals = /* @__PURE__ */ new Map();
  phases = [];
  currentPhase;
  static global() {
    if (!_TokenTracker.instance) {
      _TokenTracker.instance = new _TokenTracker();
    }
    return _TokenTracker.instance;
  }
  /** Reset all tracked data (useful for tests). */
  static reset() {
    _TokenTracker.instance = void 0;
  }
  ...
  startPhase(name) {
    ...
    this.currentPhase = { name, startedAt: Date.now(), models: /* @__PURE__ */ new Map() };
  }
  /** End the current phase, log its token report, and archive it. */
  endPhase() {
    if (!this.currentPhase) return void 0;
...
`);
  ...
  logPhaseReport(snapshot) {
    const phaseTotal = sumPhase(snapshot);
    if (phaseTotal.calls === 0) return;
    const elapsed = snapshot.endedAt ? ` in ${Math.round((snapshot.endedAt - snapshot.startedAt) / 1e3)}s` : "";
